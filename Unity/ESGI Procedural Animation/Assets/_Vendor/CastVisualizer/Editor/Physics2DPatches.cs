        ...
        partial class PhysicsScene2DPatches
        {
            ...
            static void Linecast_Internal_Postfix(Vector2 start, Vector2 end, RaycastHit2D __result)
            {
                ...
                if (__result.collider != null)
                {
                    ...
                    Instance.hitsToRender.Add((__result.point, Vector3.back));
                }

                Instance.raysToRender.Add((new Ray(start, dir), distance));
            }

#if (UNITY_2017 || UNITY_2018)
            // extern private static int LinecastNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 start, Vector2 end, ContactFilter2D contactFilter, [Out] RaycastHit2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("LinecastNonAlloc_Internal")]
            static void LinecastNonAlloc_Internal_Prefix(Vector2 start, Vector2 end)
            {
                Vector2 dir = end - start;
                Instance.raysToRender.Add((new Ray(start, dir), dir.magnitude));
            }

            [HarmonyPostfix]
            [HarmonyPatch("LinecastNonAlloc_Internal")]
            static void LinecastNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] RaycastHit2D[] results)
            {
                foreach (RaycastHit2D hit in results)
                {
                    if (hit.collider != null)
                    {
                        Instance.hitsToRender.Add((hit.point, Vector3.back));
                    }
                }
            }
...
#region Raycast
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            static void Raycast_Internal_Postfix(RaycastHit2D __result, Vector2 origin, Vector2 direction, float distance)
            {
                ...
                Instance.raysToRender.Add((new Ray(origin, direction), safeDistance));
            }

#if (UNITY_2017 || UNITY_2018)
            // extern private static int RaycastNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 origin, Vector2 direction, float distance, ContactFilter2D contactFilter, [Out] RaycastHit2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("RaycastNonAlloc_Internal")]
            static void RaycastNonAlloc_Internal_Prefix(Vector2 origin, Vector2 direction, float distance)
            {
                float safeDistance = (float.IsInfinity(distance)) ? CastVisualizerManager.INF_RAY_DRAW_LENGHT : distance;
            ...
            [HarmonyPostfix]
            [HarmonyPatch("RaycastNonAlloc_Internal")]
            static void RaycastNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] RaycastHit2D[] results)
            {
                foreach (RaycastHit2D hit in results)
                {
                    if (hit.collider != null)
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            static void BoxCast_Internal_Postfix(RaycastHit2D __result, Vector2 origin, Vector2 size, float angle, Vector2 direction, float distance)
            {
                ...
                if (__result.collider != null)
                {
                    ...
                    Instance.meshHits.Add((Physics2DMeshShape.Box, Matrix4x4.TRS(origin + direction * safeDistance, rotation, size)));
                }

                Instance.meshesToRender.Add((Physics2DMeshShape.Box, Matrix4x4.TRS(origin, rotation, size), direction * safeDistance));
            }

#if (UNITY_2017 || UNITY_2018)
            // extern private static int BoxCastNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 origin, Vector2 size, float angle, Vector2 direction, float distance, ContactFilter2D contactFilter, [Out] RaycastHit2D[] results);
            [HarmonyPrefix]
            [HarmonyPatch("BoxCastNonAlloc_Internal")]
            static void BoxCastNonAlloc_Internal_Prefix(Vector2 origin, Vector2 size, float angle, Vector2 direction, float distance)
            {
                float safeDistance = (float.IsInfinity(distance)) ? CastVisualizerManager.INF_RAY_DRAW_LENGHT : distance;

                Instance.meshesToRender.Add((Physics2DMeshShape.Box, Matrix4x4.TRS(origin, Quaternion.Euler(0.0f, 0.0f, angle), size), direction * safeDistance));
            }

            [HarmonyPostfix]
            [HarmonyPatch("BoxCastNonAlloc_Internal")]
            static void BoxCastNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] RaycastHit2D[] results, Vector2 origin, Vector2 size, float angle, Vector2 direction, float distance)
            {
                foreach (RaycastHit2D hit in results)
                {
                    float safeDistance = (float.IsInfinity(distance)) ? CastVisualizerManager.INF_RAY_DRAW_LENGHT : distance;
                    ...
                    {
                        Instance.hitsToRender.Add((hit.point, Vector3.back));
                        Instance.meshHits.Add((Physics2DMeshShape.Box, Matrix4x4.TRS(origin + direction * hit.distance, rotation, size)));
                    }
                }
            }
#elif UNITY_2019_1_OR_NEWER
            ...
            {
                Instance.meshesToRender.Add((Physics2DMeshShape.Box, Matrix4x4.TRS(point, Quaternion.Euler(0.0f, 0.0f, angle), size), Vector3.zero));
            }
            ...
            static void OverlapBox_Internal_Postfix(Collider2D __result)
            {
                if (__result != null)
                {
                    Instance.colliderHits.Add(__result);
                }
            }

#if (UNITY_2017 || UNITY_2018)
            // extern private static int OverlapBoxNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 point, Vector2 size, float angle, ContactFilter2D contactFilter, [Out] Collider2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapBoxNonAlloc_Internal")]
            static void OverlapBoxNonAlloc_Internal_Prefix(Vector2 point, Vector2 size, float angle)
            {
                Instance.meshesToRender.Add((Physics2DMeshShape.Box, Matrix4x4.TRS(point, Quaternion.Euler(0.0f, 0.0f, angle), size), Vector3.zero));
            }
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapBoxNonAlloc_Internal")]
            static void OverlapBoxNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] Collider2D[] results)
            {
                foreach (Collider2D collider in results)
                {
                    if (collider != null)
                    {
                        Instance.colliderHits.Add(collider);
                    }
                }
            }
#elif UNITY_2019_1_OR_NEWER
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            static void CircleCast_Internal_Postfix(RaycastHit2D __result, Vector2 origin, float radius, Vector2 direction, float distance)
            {
                ...
                if (__result.collider != null)
                {
                    ...
                    Instance.meshHits.Add((Physics2DMeshShape.Circle, Matrix4x4.TRS(origin + direction * safeDistance, Quaternion.identity, size)));
                }

                Instance.meshesToRender.Add((Physics2DMeshShape.Circle, Matrix4x4.TRS(origin, Quaternion.identity, size), direction * safeDistance));
            }

#if (UNITY_2017 || UNITY_2018)
            // extern private static int CircleCastNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 origin, float radius, Vector2 direction, float distance, ContactFilter2D contactFilter, [Out] RaycastHit2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("CircleCastNonAlloc_Internal")]
            static void CircleCastNonAlloc_Internal_Prefix(Vector2 origin, float radius, Vector2 direction, float distance)
            {
                float safeDistance = (float.IsInfinity(distance)) ? CastVisualizerManager.INF_RAY_DRAW_LENGHT : distance;
                Vector3 size = new Vector3(radius, radius, radius);
            ...
            [HarmonyPostfix]
            [HarmonyPatch("CircleCastNonAlloc_Internal")]
            static void CircleCastNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] RaycastHit2D[] results, Vector2 origin, float radius, Vector2 direction, float distance)
            {
                foreach (RaycastHit2D hit in results)
                {
                    float safeDistance = (float.IsInfinity(distance)) ? CastVisualizerManager.INF_RAY_DRAW_LENGHT : distance;
                    ...
                    {
                        Instance.hitsToRender.Add((hit.point, Vector3.back));
                        Instance.meshHits.Add((Physics2DMeshShape.Circle, Matrix4x4.TRS(origin + direction * hit.distance, Quaternion.identity, size)));
                    }
                }
            }
#elif UNITY_2019_1_OR_NEWER
            ...
            static void OverlapCircle_Internal_Prefix(Vector2 point, float radius)
            {
                ...
                Instance.meshesToRender.Add((Physics2DMeshShape.Circle, Matrix4x4.TRS(point, Quaternion.identity, size), Vector3.zero));
            }
...
#if (UNITY_2017 || UNITY_2018)
            // extern private static int OverlapCircleNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 point, float radius, ContactFilter2D contactFilter, [Out] Collider2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapCircleNonAlloc_Internal")]
            static void OverlapCircleNonAlloc_Internal_Prefix(Vector2 point, float radius)
            {
                Vector3 size = new Vector3(radius, radius, radius);
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapCircleNonAlloc_Internal")]
            static void OverlapCircleNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] Collider2D[] results)
            {
                foreach (Collider2D hit in results)
                {
                    if (hit != null)
                    {
                        Instance.colliderHits.Add(hit);
                    }
                }
            }
#elif UNITY_2019_1_OR_NEWER
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            static void CapsuleCast_Internal_Postfix(RaycastHit2D __result, Vector2 origin, Vector2 size, CapsuleDirection2D capsuleDirection, float angle, Vector2 direction, float distance)
            {
                ...
                if (__result.collider != null)
                {
                    ...
                    Instance.meshHits.Add((Physics2DMeshShape.Capsule, new Matrix4x4(origin + direction * safeDistance, dataVec, dataVec2, Vector3.zero)));
                }

                Instance.meshesToRender.Add((Physics2DMeshShape.Capsule, new Matrix4x4(origin, dataVec, dataVec2, Vector3.zero), direction * safeDistance));
            }

#if (UNITY_2017 || UNITY_2018)
            // extern private static int CapsuleCastNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 origin, Vector2 size, CapsuleDirection2D capsuleDirection, float angle, Vector2 direction, float distance, ContactFilter2D contactFilter, [Out] RaycastHit2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("CapsuleCastNonAlloc_Internal")]
            static void CapsuleCastNonAlloc_Internal_Prefix(Vector2 origin, Vector2 size, CapsuleDirection2D capsuleDirection, float angle, Vector2 direction, float distance)
            {
                float safeDistance = (float.IsInfinity(distance)) ? CastVisualizerManager.INF_RAY_DRAW_LENGHT : distance;
            ...
            [HarmonyPostfix]
            [HarmonyPatch("CapsuleCastNonAlloc_Internal")]
            static void CapsuleCastNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] RaycastHit2D[] results, Vector2 origin, Vector2 size, CapsuleDirection2D capsuleDirection, float angle, Vector2 direction, float distance)
            {
                foreach (RaycastHit2D hit in results)
                {
                    if (hit.collider != null)
                    {
                        ...
                        Instance.hitsToRender.Add((hit.point, Vector3.back));
                        Instance.meshHits.Add((Physics2DMeshShape.Capsule, new Matrix4x4(origin + direction * hit.distance, dataVec, dataVec2, Vector3.zero)));
                    }
                }
            }
#elif UNITY_2019_1_OR_NEWER
            ...
            static void OverlapCapsule_Internal_Prefix(Vector2 point, Vector2 size, CapsuleDirection2D direction, float angle)
            {
                ...
                Instance.meshesToRender.Add((Physics2DMeshShape.Capsule, new Matrix4x4(point, dataVec, dataVec2, Vector3.zero), Vector3.zero));
            }
...
#if (UNITY_2017 || UNITY_2018)
            // extern private static int OverlapCapsuleNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 point, Vector2 size, CapsuleDirection2D direction, float angle, ContactFilter2D contactFilter, [Out] Collider2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapCapsuleNonAlloc_Internal")]
            static void OverlapCapsuleNonAlloc_Internal_Prefix(Vector2 point, Vector2 size, CapsuleDirection2D direction, float angle)
            {
                Vector3 dataVec = new Vector3(size.x, size.y, angle);
                Vector3 dataVec2 = new Vector3((float)direction, (float)direction, (float)direction);
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapCapsuleNonAlloc_Internal")]
            static void OverlapCapsuleNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out]Collider2D[] results)
            {
                foreach (Collider2D collider in results)
                {
                    if (collider != null)
...
#region Pointcast
        ...
        partial class PhysicsScene2DPatches
        {
            ...
            {
                Instance.meshesToRender.Add((Physics2DMeshShape.Point, Matrix4x4.TRS(point, Quaternion.identity, Vector3.one), Vector3.zero));
            }
...
#if (UNITY_2017 || UNITY_2018)
            // extern private static int OverlapPointNonAlloc_Internal(PhysicsScene2D physicsScene, Vector2 point, ContactFilter2D contactFilter, [Out] Collider2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapPointNonAlloc_Internal")]
            static void OverlapPointNonAlloc_Internal_Prefix(Vector2 point)
            {
                Instance.meshesToRender.Add((Physics2DMeshShape.Point, Matrix4x4.TRS(point, Quaternion.identity, Vector3.one), Vector3.zero));
            }
            ...
            [HarmonyPostfix]
            [HarmonyPatch("OverlapPointNonAlloc_Internal")]
            static void OverlapPointNonAlloc_Internal_Postfix([System.Runtime.InteropServices.Out] Collider2D[] results)
            {
                foreach (Collider2D collider in results)
                {
                    if (collider != null)
...
#region Collidercast
        ...
        {
#if (UNITY_2017 || UNITY_2018)
            // extern private static int OverlapCollider_Internal([NotNull] Collider2D collider, ContactFilter2D contactFilter, [Out] Collider2D[] results)
            [HarmonyPrefix]
            [HarmonyPatch("OverlapCollider_Internal")]
            static void OverlapCollider_Internal_Prefix(Collider2D collider)
            {
                Instance.colliderCasts.Add(collider);
            }

            [HarmonyPostfix]
            [HarmonyPatch("OverlapCollider_Internal")]
            static void OverlapCollider_Internal_Postfix([System.Runtime.InteropServices.Out] Collider2D[] results, Collider2D collider)
            {
                foreach (Collider2D coll in results)
                {
                    if (coll != null)
                    {
                        Instance.colliderHits.Add(coll);
                    }
                }
            }
