        ...
        partial class PhysicsScenePatches
        {
            ...
            static void Internal_Raycast_Postfix(Ray ray, float maxDistance, ref RaycastHit hit)
            {
                ...
                if (hit.collider != null)
                {
                    ...
                    Instance.hitsToRender.Add((hit.point, hit.normal));
                }
                Instance.raysToRender.Add((ray, distance));
            }
        ...
        partial class PhysicsScenePatches
        {
            ...
            static void Query_BoxCast_Postfix(Vector3 center, Vector3 halfExtents, Vector3 direction, Quaternion orientation, float maxDistance, ref RaycastHit outHit)
            {
                ...
                if (outHit.collider != null)
                {
                    ...
                    Instance.hitsToRender.Add((outHit.point, outHit.normal));

                    Instance.meshHits.Add((PhysicsMeshShape.Box, Matrix4x4.TRS(center + direction * distance, orientation, extends)));
                }

                Instance.meshesToRender.Add((PhysicsMeshShape.Box, Matrix4x4.TRS(center, orientation, extends), direction * distance));
            }
            ...
            static void Internal_BoxCastNonAlloc_Postfix(RaycastHit[] raycastHits, Vector3 center, Vector3 halfExtents, Vector3 direction, Quaternion orientation)
            {
                ...
                foreach (RaycastHit hit in raycastHits)
                {
                    ...
                    {
                        Instance.hitsToRender.Add((hit.point, hit.normal));
                        Instance.meshHits.Add((PhysicsMeshShape.Box, Matrix4x4.TRS(center + direction * hit.distance, orientation, extends)));
                    }
                }
            }
            ...
            {
                Vector3 extends = halfExtents * 2.0f;
                Instance.meshesToRender.Add((PhysicsMeshShape.Box, Matrix4x4.TRS(center, orientation, extends), Vector3.zero));
            }
        ...
        partial class PhysicsScenePatches
        {
            ...
            static void Query_SphereCast_Postfix(Vector3 origin, float radius, Vector3 direction, float maxDistance, ref RaycastHit hitInfo)
            {
                ...
                if (hitInfo.collider != null)
                {
                    ...
                    Instance.hitsToRender.Add((hitInfo.point, hitInfo.normal));

                    Instance.meshHits.Add((PhysicsMeshShape.Sphere, Matrix4x4.TRS(origin + direction * distance, Quaternion.identity, extends)));
                }

                Instance.meshesToRender.Add((PhysicsMeshShape.Sphere, Matrix4x4.TRS(origin, Quaternion.identity, extends), direction * distance));
            }
            ...
            static void Internal_SphereCastNonAlloc_Postfix(RaycastHit[] raycastHits, Vector3 origin, float radius, Vector3 direction)
            {
                ...
                foreach (RaycastHit hit in raycastHits)
                {
                    ...
                    {
                        Instance.hitsToRender.Add((hit.point, hit.normal));
                        Instance.meshHits.Add((PhysicsMeshShape.Sphere, Matrix4x4.TRS(origin + direction * hit.distance, Quaternion.identity, extends)));
                    }
                }
            }
            ...
            {
                Vector3 extends = new Vector3(radius, radius, radius);
                Instance.meshesToRender.Add((PhysicsMeshShape.Sphere, Matrix4x4.TRS(position, Quaternion.identity, extends), Vector3.zero));
            }
        ...
        partial class PhysicsScenePatches
        {
            ...
            static void Query_CapsuleCast_Postfix(Vector3 point1, Vector3 point2, float radius, Vector3 direction, float maxDistance, ref RaycastHit hitInfo)
            {
                ...
                if (hitInfo.collider != null)
                {
                    ...
                    Instance.meshHits.Add((PhysicsMeshShape.Capsule, new Matrix4x4(point1 + direction * distance, point2 + direction * distance, extends, Vector4.zero)));
                }

                Instance.meshesToRender.Add((PhysicsMeshShape.Capsule, new Matrix4x4(point1, point2, extends, Vector4.zero), direction * distance));
            }
            ...
            static void Internal_CapsuleCastNonAlloc_Prefix(Vector3 p0, Vector3 p1, float radius, Vector3 direction, float maxDistance)
            {
                ...
                Instance.meshesToRender.Add((PhysicsMeshShape.Capsule, mat, direction * distance));
            }
            ...
            static void Internal_CapsuleCastNonAlloc_Postfix(RaycastHit[] raycastHits, Vector3 p0, Vector3 p1, float radius, Vector3 direction)
            {
                ...
                foreach (RaycastHit hit in raycastHits)
                {
                    if (hit.collider != null)
                    {
                        ...
                        Instance.hitsToRender.Add((hit.point, hit.normal));
                        Instance.meshHits.Add((PhysicsMeshShape.Capsule, mat));
                    }
                }
            }
            ...
            static void OverlapCapsuleNonAlloc_Internal_Prefix(Vector3 point0, Vector3 point1, float radius)
            {
                ...
                Instance.meshesToRender.Add((PhysicsMeshShape.Capsule, mat, Vector3.zero));
            }
